...
class AllTestsCase(TestCase):
    ...
    def test_home_routes_views(self):
        ...
        self.assertTemplateUsed(response, template_name='routes/home.html') # проверка какой используется шаблон
        self.assertEqual(response.resolver_match.func, routes_view.home) # проверка какая функция выполняется

    def test_cbv_detail_views(self): # какая view используется для детализации записи города
    ...
    def test_valid_route_form(self):  # проверка формы
        data = {'from_city': self.city_A.id, 'to_city': self.city_B.id,
                'cities': [self.city_E.id, self.city_D.id], 'travelling_time': 9}
        form = RouteForm(data=data)
        self.assertTrue(form.is_valid())

    def test_invalid_route_form(self):  # проверка формы, нет одного обязательного поля
        data = {'from_city': self.city_A.id, 'to_city': self.city_B.id,
                'cities': [self.city_E.id, self.city_D.id], }
        form = RouteForm(data=data)
        self.assertFalse(form.is_valid())

    def test_invalid_int_float_route_form(self):  # проверка формы, вместо int - float
        data = {'from_city': self.city_A.id, 'to_city': self.city_B.id,
                'cities': [self.city_E.id, self.city_D.id], 'travelling_time': 9.45 }
        form = RouteForm(data=data)
        self.assertFalse(form.is_valid())

    def test_message_error_more_time(self):  # наличие сообщений об ошибках
        data = {'from_city': self.city_A.id, 'to_city': self.city_E.id,
                'cities': [self.city_C.id], 'travelling_time': 9}
        response = self.client.post('/find_routes/', data)
        self.assertContains(response, 'Время в пути больше заданного', 1, 200)

    def test_message_error_from_cities(self):  # наличие сообщений об ошибках
        data = {'from_city': self.city_B.id, 'to_city': self.city_E.id,
                'cities': [self.city_C.id], 'travelling_time': 349}
        response = self.client.post('/find_routes/', data)
